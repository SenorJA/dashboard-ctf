...
> Última actualización: 15 Sep 2026 — MIRV v6.2 | 36 módulos | 254+ endpoints | 4711 tests | 31 tabs | **88 skills** | main.py 100%
...
> ✅ **Pack 3 — Scheduler daemon + Finding lifecycle** (15 Sep 2026): **(1) Daemon server-side de scheduler (idea A)** — `backend/scheduler.py` con `_TOOL_COMMANDS` (14 tools con templates `{target}`) + `get_command()`; `due_jobs(include_tools=, require_target=)` filtra antes del auto-advance single-trigger (el daemon solo avanza tools mapeadas con target fijo; los jobs unmapped o sin target quedan due para el poll del browser sin perder el ciclo). `main.py`: `_exec_tool_command()` (SSH compartido vía `asyncio.to_thread`, out cap 8 KB/`truncated_out`, stderr 4 KB), `_count_findings_in_output()` (heurísticas nmap/gobuster-dirb-ffuf-wfuzz-feroxbuster/nikto/enum4linux/smbclient/whatweb/otras), `_scheduler_loop()` (ciclo 5 s, spawn en startup, cancel en shutdown) → los scans programados se ejecutan **sin browser conectado** y `record_run()` guarda resultado+findings. Endpoint `GET /api/scheduler/status` (daemon_running/mapped_tools) + badge ⚙ en tab Scheduler (`refreshSchedulerDaemon`). **(2) Finding lifecycle + binding a assessment (idea B)** — columnas `lifecycle_status` (open/confirmed/accepted/fixed/verified) + `assessment_id` en tabla findings (migración `ADD COLUMN IF NOT EXISTS` en `SCHEMA_SQL`; `save_finding`/`save_findings_bulk`/`list_findings` con los campos; `database.update_finding()` whitelist). Endpoints `PATCH /api/findings/{id}` (400 lifecycle inválido/campo desconocido, 404), `GET /api/findings/assessment/{id}`, filtros `lifecycle_status`/`assessment_id` en `GET /api/findings`. Frontend: badge + select de estado por finding (`setFindingLifecycle`), filtro 🗂 por estado, botón 📎/✓ bind a assessment activa (`bindFindingToAssessment` con rollback optimista). Tests: `test_scheduler_daemon.py` (27) + `test_finding_lifecycle.py` (19) → suite completa CI: **4711 passed, 0 failed** ✅.
