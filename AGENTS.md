...
| **Notifications** | `GET /api/notifications/providers` (masked), `POST /api/notifications/config`, `DELETE /api/notifications/config/{provider}`, `POST /api/notifications/test` (síncrono 200/502), `POST /api/notifications/send` (fire-and-forget). Hooks opt-in: findings high/critical + runs de scheduler (daemon + record) |
