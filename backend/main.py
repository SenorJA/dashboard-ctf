...
class PdfRequest(BaseModel):
    """Legacy PDF request — backward compatible."""
    content: str
    title: str = "M.I.R.V. Report"
    author: str = "M.I.R.V."


class PdfFindingModel(BaseModel):
    """Single finding for professional PDF."""
    title: str
    severity: str = "info"
    detail: str = ""
    target: str = ""
...
class PdfProfessionalRequest(BaseModel):
    """Professional PDF generation request."""
...
def _parse_markdown_to_sections(content: str, title: str) -> list:
    """Parse markdown text into PdfSectionModel list for the professional engine."""
    from backend.pdf_engine import PdfSection
    sections = []
    ...
    for line in content.split("\n"):
        ...
        if stripped.startswith("## "):
            ...
            current_section = PdfSectionModel(heading=stripped[3:].strip())
            current_content_lines = []
    ...
    if not sections and content.strip():
        sections.append(PdfSectionModel(heading=title, content=content.strip()))
...
async def generate_pdf(req: PdfRequest):
    """Generate a PDF from markdown-like text (legacy endpoint, backward compatible)."""
    try:
        ...
        report = PdfReport(
            title=req.title,
            author=req.author,
            executive_summary=f"Report generated from markdown content.",
            sections=[],
        )

        # Parse markdown into sections
        section_models = _parse_markdown_to_sections(req.content, req.title)
        for sm in section_models:
            from backend.pdf_engine import PdfSection
            report.sections.append(PdfSection(
                heading=sm.heading,
                content=sm.content,
            ))
...
@app.post("/api/generate-pdf-professional")
async def generate_pdf_professional(req: PdfProfessionalRequest):
    """Generate a professional PDF with cover page, TOC, findings table, severity colors."""
    try:
        ...
        sections = []
        if req.sections:
            for sm in req.sections:
                if isinstance(sm, dict):
                    sections.append(PdfSection(
                        heading=sm.get("heading", ""),
                        content=sm.get("content", ""),
                    ))
                elif hasattr(sm, 'heading'):
                    sections.append(PdfSection(
                        heading=sm.heading,
                        content=getattr(sm, 'content', ''),
                    ))
        elif req.content:
            section_models = _parse_markdown_to_sections(req.content, req.title)
            for sm in section_models:
                sections.append(PdfSection(heading=sm.heading, content=sm.content))

        # Build findings
        findings = []
        for fm in req.findings:
            if isinstance(fm, dict):
                findings.append(PdfFinding(
                    title=fm.get("title", "Untitled"),
                    severity=fm.get("severity", "info"),
                    detail=fm.get("detail", ""),
                    target=fm.get("target", req.target),
                    tool=fm.get("tool", ""),
                    recommendation=fm.get("recommendation", ""),
                    references=fm.get("references", []),
                ))
            elif hasattr(fm, 'title'):
                findings.append(PdfFinding(
                    title=fm.title,
                    severity=getattr(fm, 'severity', 'info'),
                    detail=getattr(fm, 'detail', ''),
                    target=getattr(fm, 'target', req.target),
                    tool=getattr(fm, 'tool', ''),
                    recommendation=getattr(fm, 'recommendation', ''),
                    references=getattr(fm, 'references', []),
                ))
        ...
        from fastapi.responses import StreamingResponse
        filename = f"{req.title[:50].replace(' ', '_')}.pdf"
        return StreamingResponse(
            io.BytesIO(pdf_bytes),
            media_type="application/pdf",
            headers={
