...
> Última actualización: 21 Sep 2026 — MIRV v5.6 | desktop shell Tauri (tray + auto-updater) | multi-terminal SSH | notifications hub | API token auth opt-in | asset inventory per engagement | scheduler daemon + finding lifecycle | 295+ endpoints | 4790+ tests | 31 tabs | main.py 100%

## ✅ Completado

### Fase 8 — Mejoras (Pack 3 y siguientes)
...
- [x] **Feature F — Terminal multi-sesión**: backend `/ws` ya crea un shell paramiko independiente por conexión (sin estado compartido) — garantizado por `TestWebSocketMultiSession` en `test_main_websocket_gaps.py` (2 conexiones concurrentes con canales A/B: vida por target, output aislado sin cross-talk, comandos enrutados a su canal, `invoke_shell.call_count == 2`). Frontend aditivo sin tocar el pipeline del terminal principal: barra `➕ Console` + chips `📡 cN`, cada consola autocontenida (WS propio sobre el perfil activo, output ANSI-stripped, input Enter, ▶/⏹ toggle, ✕ con cleanup de WS). JS verificado con `node --check`.
...
- [x] **Feature E — Desktop shell Tauri v2 (tray + auto-updater)**: `desktop/` lanza el backend como sidecar embebido (`externalBin binaries/mirv-backend`) + ventana 1280×800 WebViewEdge. Cierre de ventana → bandeja del sistema (`menu_on_left_click=false`, menú Show/Quit; "Quit" flipa `AtomicBool` para salida real que mata el sidecar). Auto-updater con `tauri-plugin-updater`: check en cada arranque contra GitHub Releases `latest.json` → `download_and_install` → `restart`. **Signing minisign** (par generada 20 Sep 2026; privada gitignored en `desktop/.tauri/`, pubkey en `tauri.conf.json`). `desktop-build.yml`: PyInstaller → sidecar → npm → Rust → `tauri-action --bundles msi` (windows-latest). Nota CI: `createUpdaterArtifacts` NO está en la config base (sin clave el bundler falla "public key found but no private key") — el workflow lo inyecta vía `--config src-tauri/tauri.updater.json` solo en **tags `v*` con `TAURI_SIGNING_PRIVATE_KEY`**. Verificado: `cargo check` local limpio (toolchain Rust 1.98 + VS BuildTools 17.14, placeholder del sidecar gitignored en `binaries/`) + Desktop Build CI verde en `main`.
