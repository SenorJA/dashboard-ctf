...
window.ctfLoadScore = async function () {
    try {
        ...
        const s = data.data;
        const elSolved = document.getElementById('ctf-score-solved');
        const elTotal = document.getElementById('ctf-score-total');
        const elPoints = document.getElementById('ctf-score-points');
        const elTotalPts = document.getElementById('ctf-score-total-pts');
        if (elSolved) elSolved.textContent = s.solved || 0;
        if (elTotal) elTotal.textContent = s.total || 0;
        if (elPoints) elPoints.textContent = s.points || 0;
        if (elTotalPts) elTotalPts.textContent = s.total_points || 0;
