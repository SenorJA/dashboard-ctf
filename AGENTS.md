...
| `assets.py` | ~470 | Asset inventory per engagement — registry thread-safe dedup `(assessment_id, kind, address)`, kinds host/domain/service/endpoint/other, status active/potential/out-of-scope/infrastructure/compromised, open ports + tech stack + `findings_count`, `ingest_findings()` auto-populate (IP→host / hostname→domain / URLs via urlparse / ports / services / endpoint for paths), summary, export/import, cascade `delete_assets_for_assessment`, opt-in `workspace_store` persistence (key `assets`) | 22 | — |
| `api_auth.py` | ~130 | Opt-in API token guard for `/api/*` — sources: `Authorization: Bearer`, `X-MIRV-Token`, `mirv_token` httpOnly cookie (set on SPA serve); config env `MIRV_API_TOKEN` (wins) or file `MIRV_API_TOKEN_FILE` (default `backend/data/api_token.txt` first line); constant-time compare, `is_exempt()`/`token_source()`/`mask()`, disabled when no token (opt-in, hermetic tests) | 17 | — |
...
| **Assets** | `GET /api/assets` (assessment_id/kind/status filters), `POST /api/assets`, `GET /api/assets/export`, `POST /api/assets/import`, `GET /api/assets/by-assessment/{id}`, `GET /api/assets/summary`, `POST /api/assets/ingest` (batch findings → `{created,updated,skipped,ports,services,errors}`), `GET/PUT/DELETE /api/assets/{id}` |
| **Auth** | `GET /api/auth/status` (público: enabled/source/masked_token), `GET /api/auth/token` (guardado: echo del token). Guard opt-in middleware solo `/api/*` — sources `Authorization: Bearer` | `X-MIRV-Token` | cookie `mirv_token`; desactivado sin `MIRV_API_TOKEN`/archivo |
...
- **81 test files** in `backend/tests/` (83 counting `test_scheduler_daemon.py` + `test_finding_lifecycle.py` + `test_assets.py` + `test_api_auth.py`)
- **~4750 tests** collected (296 now in `test_main_gaps.py` + 19 in `test_main_websocket_gaps.py` + 30 `test_assessments.py` + 43 `test_scheduler.py` + 27 `test_scheduler_daemon.py` + 19 `test_finding_lifecycle.py` + 22 `test_assets.py` + 17 `test_api_auth.py` + 22 `test_workspace_store.py`)
