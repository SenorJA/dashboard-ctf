...
window.kbSearch = async function () {
    ...
    container.innerHTML = '<div class="text-[10px] text-gray-400 text-center py-4">Searching...</div>';
...
function kbRender(data, container) {
    ...
    if (cves.length === 0 && mitre.length === 0) {
        container.innerHTML = '<div class="text-[10px] text-gray-400 text-center py-4">No results</div>';
        return;
    }
    ...
    if (cves.length > 0) {
        html += '<div class="text-[9px] text-gray-400 uppercase tracking-wider mb-1 mt-2">CVEs (' + cves.length + ')</div>';
        html += cves.map(c => {
            ...
            return '<div class="bg-deep/50 border border-gray-800 rounded p-2 mb-1 text-[10px]">' +
                '<div class="flex items-center justify-between">' +
                    ...
                    '</div>' +
                    '<span class="text-gray-400 text-[9px]">' + (c.tools || []).join(', ') + '</span>' +
                '</div>' +
                '<div class="text-gray-400 mt-0.5">' + c.description + '</div>' +
                '<div class="text-gray-400 text-[9px]">Affects: ' + c.affected + (c.exploit_available ? ' | Exploit available' : '') + '</div>' +
            '</div>';
        }).join('');
    }

    if (mitre.length > 0) {
        html += '<div class="text-[9px] text-gray-400 uppercase tracking-wider mb-1 mt-3">MITRE ATT&CK (' + mitre.length + ')</div>';
        html += mitre.map(t => {
            return '<div class="bg-deep/50 border border-gray-800 rounded p-2 mb-1 text-[10px]">' +
                '<div class="flex items-center justify-between">' +
                    '<div class="flex items-center gap-2">' +
                ...
                (t.examples ? '<div class="text-gray-400 text-[9px] mt-0.5">Examples: ' + t.examples.join(', ') + '</div>' : '') +
            '</div>';
        }).join('');
    }
    ...
    if (result) {
        ...
        overlay.innerHTML = `<div class="bg-deep border border-cyan-500/30 rounded-lg max-w-xl w-full mx-4 max-h-[70vh] overflow-y-auto p-4 shadow-2xl">
            <div class="flex items-center justify-between mb-3">
                ...
                <button onclick="this.closest('#kb-ai-overlay').remove()" class="text-gray-400 hover:text-gray-400 text-[18px] leading-none">&times;</button>
            </div>
            ...
            <div class="mt-3 pt-2 border-t border-gray-800 flex justify-end">
                <button onclick="this.closest('#kb-ai-overlay').remove()" class="text-[9px] text-gray-400 hover:text-gray-400">Close</button>
            </div>
        </div>`;
