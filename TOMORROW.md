...
> Última actualización: 26 Sep 2026 — MIRV v6.2 | 39 módulos | 272+ endpoints | 4788 tests | 31 tabs | **88 skills** | main.py 100% | **Desktop v3.0.1 release firmada** | **Login + activación token API (Pack 8)** | 3 workflows (CI + Deploy + Desktop Build) verdes
...
> ✅ **T4+T6+T7 — Planes Op Admiral + key rotation + contraste JS** (2 Sep 2026): **T4** auto-guardar/cargar planes de Op Admiral (frontend `Save Plan`/`📚`, lista persisted en `/api/plans`, auto-save al generar plan, load/delete, `opd-plan-list` con aria-live; fix backend `list_mission_plans` parsea `steps` JSON-string → array). **T6** contraste P1 residual en render dinámico de módulos JS secundarios (`text-gray-600/700→400` en credentials/ctf/forensics/kb/mobile/scope/swarm/main — 0 residuales). **T7** rotación de key `secret_store`: helpers `encrypt_with`/`decrypt_with` + `reencrypt_app_credentials(old_key)` en database + `POST /api/credentials/reencrypt` en main. Suite completa: **4385 passed, 0 failed**.
> ✅ **T5 — MIRV Desktop (PyInstaller sidecar + Tauri scaffold)** (3 Sep 2026): **Phase 1 backend empaquetado con PyInstaller** — `backend/mirv-backend.spec` (bundles `skills/`, `plugins/`, `frontend/`; wrappers tolerantes a deps ausentes) + `backend/build_backend.bat`. `main.py`: `IS_PACKAGED`/`TAURI_MODE`/`_resolve_frontend_dir()` (frozen-aware) + `__main__` con `--host`/`--port`/`MIRV_PORT` y paso del **app object directo a uvicorn** cuando está congelado (fix "Could not import module 'main'"). Validador del exe real: `/api/health` + `/api/skills` (91 skills empaquetadas) 200 OK. **Phase 2/3 Tauri scaffold** — `desktop/` completo (package.json + prebuild→sync-frontend.mjs copia `frontend/*`→`desktop/src/`, src-tauri {Cargo.toml, tauri.conf.json con CSP, build.rs, src/main.rs sidecar spawn + health-check backoff + kill-on-window-destroy}, build_desktop.bat, README, icons). `main.v2.js` top-level: detección `IS_TAURI`, `API_BASE`/`WS_URL` inyección + monkeypatch condicional de `window.fetch` (no-op en browser). Suite completa: **4387 passed, 0 failed** (+2: guard frozen/tauri test).
...
> ✅ **Improvements pack — 5 features planeadas** (10 Sep 2026): **(1) Hak5 tools** — plantillas por dispositivo (`window.insertHak5Template`, 7 templates), validador por lenguaje (`validateHak5Payload` DuckyScript/bash/JS/MicroPython, warn sin bloquear) y descarga (`.ext` de cada device). **(2) Dashboard Home** — pestaña 🏠 `tab-home` con KPIs (backend health+uptime, findings, targets+conexiones, coverage pass-ratio, SIEM, intel alerts, CPU/RAM/disco) vía `refreshDashboard()` (8 fetches paralelos) + quick actions + refresh wrapper en `switchTab('home')`; tabs 28→29. **(3) Findings export estructurado** — `GET /api/findings/export?format=csv|sarif|html` (`_export_findings_csv/sarif/html`, **SARIF 2.1.0** con driver MIRV + level error/warning/note, HTML self-contained con badges+summary) + botones ⬇ CSV/SARIF/HTML en tab Findings (`downloadFindingsExport()`). **(4) SIEM webhook externo** — `set_webhook_url`/`get_webhook_url` en `siem.py` (validación http/s, string vacío = clear, thread-safe) + `_notify_webhook` fire-and-forget desde `_create_alert` (urllib daemon thread 5 s, payload JSON `siem-alert`) + `GET/POST/DELETE /api/siem/webhook` + card UI en tab SIEM con badge de estado. **(5) MCP OSINT #3** — `mcp_server.py` expone las 5 tools Ronda #3 (`vulnforge_osint_{code_search,cert_transparency,sigstore,urlscan,mac_lookup}`). Ya se incluían antes en AGENTS los Hak5 6 dispositivos (Packet Squirrel Mark II + Shark Jack Display). Tests nuevos: 13 SIEM webhook + 12 export endpoints + 13 MCP OSINT routing. Suite afectada: **484 passed**. ⬜ 1 fallo flaky pre-existente ambiental: `test_main_gaps.py::TestMobileApi::test_delete_not_found` (depende de `db.delete_mobile_apk` real, no relacionado).
> ✅ **Fix test ambiental flaky** (10 Sep 2026, commit tras `f1a0527`): `test_main_gaps.py::TestMobileApi::test_delete_not_found` mockea ahora también `main.db.delete_mobile_apk → False`, que era el agujero que hacía que con Supabase vivo el endpoint respondiera 200 (cuando el test espera 404 — el mock existente `mobile_delete_apk=False` se quedaba corto). `TestMobileApi` 8/8 ✅, `test_main_gaps.py` **296 passed** ✅.
> ✅ **Pack de 5 mejoras de uso** (11 Sep 2026): **(1) Terminal workflow** — historial de comandos **persistente** (`mirv_cmd_history`, flechas ↑/↓, botón 🗑 History `clearCmdHistory`), **export de sesión** (`exportSession` → `.log`/`.md`), **resumen IA** (`aiSessionSummary` → `/api/ai/chat`, botón 🤖 Summary) y **sugerencias next-step deterministas** (`computeNextSteps` + `renderNextStepSuggestions`: nmap→whatweb/gobuster/hydra-según-puertos, dir-enum→nikto/wpscan si WP, sqlmap→--dbs, DNS AXFR→dig, …; render en panel Findings→Suggestions clicable `.suggest-run-cmd` + aviso en terminal; cero IA). **(2) Charts Overview en Findings** — 3 canvas puro por severidad (colores) / top tools / top targets (`renderFindingsCharts`, `details` colapsable persistido). **(3) Cheatsheet** — modal 📖 Cheat con ~50 comandos curados por categoría + búsqueda en vivo + clic precarga con `TARGET`→target activo (`openCheatSheet/closeCheatSheet/loadCheatCommand/renderCheatSheet`). **(4) Assessments workspace** — módulo `backend/assessments.py` (in-memory thread-safe, statuses planning/in-scope/in-progress/done/archived, targets dedup, tags, límites 200/100) + 8 endpoints `GET/POST /api/assessments*` + tab 🗂️ Assessments (form inline, cards con chips de targets, add/remove por Enter/✕, status select, ⚡ Scan first→nmap, 🗑 Delete). **(5) Scheduler** — módulo `backend/scheduler.py` (jobs in-memory, intervalos 10s–7d validados, `due_jobs()` **auto-advance single-trigger**, `advance_to_now` run-now) + 7 endpoints `GET/POST/PUT/PATCH/DELETE /api/scheduler*` + `GET /api/scheduler/due` + `POST /api/scheduler/jobs/{id}/run` + tab ⏰ Scheduler (countdown, ⏸/▶, ⚡ Run now) + **polling global 10 s** que lanza `launchTool(tool_id)` directamente en el terminal. Tabs 29→**31**, rutas registradas 254→**269** (+8 assessments, +7 scheduler; medido sobre `@app.get/post/put/patch/delete` en main.py). Tests nuevos: 30 assessments + 37 scheduler (+1 run-now/404). Suite completa (CI cmd): **4641 passed** ✅. ⬜ Pre-existente no relacionado: `test_orchestrator.py::TestCallLlm::test_openai_provider_default_model_when_empty` (env, falla igual en `ff057a8`).

> ✅ **Pack 3 — Scheduler daemon + Finding lifecycle + Asset inventory** (18 Sep 2026): **(1) Daemon server-side de scheduler (idea A)** — `backend/scheduler.py` con `_TOOL_COMMANDS` (14 tools con templates `{target}`) + `get_command()`; `due_jobs(include_tools=, require_target=)` filtra antes del auto-advance single-trigger (el daemon solo avanza tools mapeadas con target fijo; los jobs unmapped o sin target quedan due para el poll del browser). `main.py`: `_exec_tool_command()` (SSH compartido vía `asyncio.to_thread`, out cap 8 KB/`truncated_out`, stderr 4 KB), `_count_findings_in_output()` (heurísticas por familia), `_scheduler_loop()` (ciclo 5 s, spawn **idempotente** en startup, cancel en shutdown) + `GET /api/scheduler/status` (daemon_running/mapped_tools/policy). Frontend: badge ⚙ daemon en tab Scheduler (`refreshSchedulerDaemon`). **(2) Finding lifecycle + binding a assessment (idea B)** — columnas `lifecycle_status` (open/confirmed/accepted/fixed/verified) + `assessment_id` en tabla findings (migración `ADD COLUMN IF NOT EXISTS` en `SCHEMA_SQL`), `database.update_finding()` whitelist; endpoints `PATCH /api/findings/{id}` (400 lifecycle inválido/campo desconocido, 404), `GET /api/findings/assessment/{id}`, filtros en `GET /api/findings`. Frontend: badge+select de estado por finding, filtro 🗂, bind 📎/✓ a assessment activa con rollback optimista. **(3) Asset inventory por engagement (idea C)** — módulo `backend/assets.py`: dataclass `Asset`, registry thread-safe con índice de dedup `(assessment_id, kind, address)`, kinds host/domain/service/endpoint/other, status active/potential/out-of-scope/infrastructure/compromised, puertos `{port,protocol,service,version}`, stack `services`, tags, `findings_count`, source manual/tool. `ingest_findings()` auto-popula: IP→host, hostname→domain, URLs vía `urlparse` (host+port), findings con `port` adjuntan puerto (dedup port/protocol), con `service`/`version` enriquecen stack, con `path` crean endpoints; merge idempotente por dedup key. Persistencia opt-in `workspace_store` (key `assets`) + `load_from_store()` en startup. 11 endpoints REST (`GET/POST /api/assets`, `export/import`, `by-assessment/{id}`, `summary`, `ingest`→`{created,updated,skipped,ports,services,errors}`, `GET/PUT/DELETE /api/assets/{id}`) + cascade `delete_assets_for_assessment()` al borrar un assessment. Frontend: panel 🖧 **Asset Inventory** en tab Assessments (chips de puertos, servicios, badges status, 🔎 findings_count, delete ✕), botones ⚡ Ingest findings / 🔄 Refresh, auto-ingest debounced 1.5 s en `addFindings` con assessment activa. Tests: `test_scheduler_daemon.py` (27) + `test_finding_lifecycle.py` (19) + `test_assets.py` (22). Suite completa CI: **4733 passed, 0 failed** ✅.

> ✅ **Pack 4 — API auth + Notifications hub** (18 Sep 2026): **(1) API token auth opt-in (idea D)** — módulo `backend/api_auth.py` (fuentes `Authorization: Bearer` | `X-MIRV-Token` | cookie httpOnly `mirv_token`; config env `MIRV_API_TOKEN` gana o archivo `MIRV_API_TOKEN_FILE`; `hmac.compare_digest`, `token_source()`, `mask()`, `is_exempt()` `/api/health`+`/api/auth/status`); middleware `ApiTokenMiddleware` solo `/api/*` (401 JSON `WWW-Authenticate: Bearer`, `X-MIRV-Auth: required`); `/ws` conserva su handshake JSON propio. +2 endpoints: `GET /api/auth/status` (público) y `/api/auth/token` (guardado, echo). **Opt-in estricto**: sin token → guard OFF, frontend funcionando vía cookie sin cambios. KPI card 🔐 API Auth en Home + `copyApiToken()`. Tests `test_api_auth.py` (17). **(2) Notification hub (idea G)** — módulo `backend/notifications.py`: proveedores Telegram/Discord/Slack/Pushover/webhook; registry en memoria (API) + fallback env `MIRV_NOTIFY_*`; enmascarado `mask()`, builders por proveedor, truncación 4000, `send()` fire-and-forget (daemon thread) + `send_sync()` bloqueante; **no-op estricto sin proveedores**. +5 endpoints (`GET /api/notifications/providers`, `POST /config`, `DELETE /config/{p}`, `POST /test` síncrono 200/502, `POST /send`). Hooks opt-in: findings high/critical (single+bulk) y runs de scheduler (record + daemon). KPI 🔔 en Home + panel providers. Tests `test_notifications.py` (37). Suite: **4787 passed, 0 failed** ✅.

> ✅ **Pack 5 — Terminal multi-sesión** (18 Sep 2026): el backend `/ws` ya crea un shell paramiko independiente por conexión (todo el estado es local a la coroutina: `ssh`, `channel`, `stop`, tasks `read_shell`/`read_ws`) → dos conexiones concurrentes son 100% independientes. Garantizado por `TestWebSocketMultiSession` (patrón direct-call con `asyncio.gather`: 2 fakes de WebSocket, canales A/B con `invoke_shell.side_effect`, asserts de no cross-talk y enrutado de comandos). Frontend: aditivo, el terminal principal y su pipeline de parsing quedan intactos — barra `➕ Console` + chips `📡 cN` en el tab Terminal; cada consola es un panel autocontenido (módulo `mircConsoles`): WebSocket propio sobre el perfil de conexión activo, output con ANSI-strip básico, input Enter, toggle ▶/⏹ y ✕ con cleanup de WS y chip. Shelf: `test_main_websocket_gaps.py` 19 passed + 1 slow deselected (20 en total).

> ✅ **Pack 6 — Desktop tray + auto-updater (Feature E)** (20 Sep 2026): el shell Tauri de `desktop/` se cierra la bandeja del sistema (close→hide, menú Show/Quit, left-click restaura; "Quit" flipa un `AtomicBool` para salida real que mata el sidecar). Auto-updater con `tauri-plugin-updater`: en cada arranque consulta GitHub Releases `latest.json`, descarga, instala y reinicia; artefacts `createUpdaterArtifacts: v1Compatible` en MSI. Signing minisign: pair generada 20 Sep 2026 (privada gitignored en `desktop/.tauri/`, pubkey commiteada en `tauri.conf.json`); `desktop-build.yml` pasa `TAURI_SIGNING_PRIVATE_KEY(_PASSWORD)` a `tauri-action`. Nota CI: `createUpdaterArtifacts` NO va en la config base — activado sin clave el bundler falla ("public key found but no private key"); el workflow lo inyecta via `--config src-tauri/tauri.updater.json` solo en tags `v*` + secret. Verificado en local y CI: toolchain Rust 1.98 + VS BuildTools 17.14 instalados (20 Sep 2026) → `cargo check` sin warnings (placeholder del sidecar en `binaries/`, gitignored); Desktop Build verde.

> ✅ **Pack 4 — API token auth opt-in** (18 Sep 2026): módulo `backend/api_auth.py` — fuentes aceptadas: `Authorization: Bearer <t>` | `X-MIRV-Token: <t>` | cookie httpOnly `mirv_token` (set al servir el SPA en `/`, el frontend funciona sin cambios). Config: env `MIRV_API_TOKEN` (gana) o archivo `MIRV_API_TOKEN_FILE` (default `backend/data/api_token.txt`, primera línea). Comparación constante-tiempo `hmac.compare_digest`, `token_source()` env/file, `mask()`, `is_exempt()` (`/api/health`, `/api/auth/status`). Middleware `ApiTokenMiddleware` solo para `/api/*` (401 con `WWW-Authenticate: Bearer` + `X-MIRV-Auth: required`, respuesta JSON `ok:false`); `/ws` conserva su handshake JSON propio. Endpoints: `GET /api/auth/status` (público: enabled/source/masked_token) y `GET /api/auth/token` (guardado: echo para copiar en Burp Bridge/scripts). **Opt-in estricto**: sin token configurado → guard OFF y comportamiento idéntico (tests herméticos intactos, 17 nuevos en `test_api_auth.py`: disabled-default, env/file/missing-file, Bearer/X-header/cookie/invalid, exempt paths, status/token endpoints, mask, env-wins-file). Frontend: KPI card 🔐 API Auth en Home (dot cyan=on, masked token) + `copyApiToken()`. Suite: **4750 passed, 0 failed** ✅.

> ✅ **Pack 7 — Desktop Fase 4 + v3.0.1 release** (21 Sep 2026): cerradas las **opciones 3 y 4** del plan desktop. **Opc 3 — identifier**: `com.mirv.app` → `com.mirv.desktop` (se elimina el warning del bundler y se prepara el upgrade path para futuras v3.0.2+). **Opc 4 — Fase 4**: ventana **splash** (frameless 480×320, radar animado, `frontend/splash.html` copiado a `desktop/src/` por `sync-frontend.mjs`) + ventana **main** con `visible:false` que aparece cuando el backend responde; **título dinámico** `M.I.R.V. v{version} — {estado}` (`refresh_status_title`); **página de error** `frontend/error.html` si el backend no queda listo en 45 s (Reintentar + auto-poll 3 s + Cerrar); **guards por label** en `on_window_event` (hide-to-tray/exit solo para `main`; la splash se cierra sin matar la app); iconos regenerados (`npm run icons`). **Versión subida a 3.0.1** (tauri.conf.json, Cargo.toml, Cargo.lock, package.json) para validar el updater end-to-end. **Releases firmadas publicadas**: **v3.0.0** (id 392882859, commit `e5e0f85`, 5 assets) y **v3.0.1** (id 393781977, commit `1f37606`, 5 assets incl. `latest.json`) → `https://github.com/SenorJA/dashboard-ctf/releases/latest/download/latest.json` devuelve version 3.0.1 (verified). Toolchain Rust local instalado (rustup 1.98.1 + VS BuildTools 2022 17.14) → `cargo check` limpio; MSI construido local; 3 workflows verdes en `main` y en el tag. gh CLI 2.101.0 instalado; PAT fine-grained del usuario en `%TEMP%\opencode\gh_token.txt`. Stack Docker local re-validado (mirv-backend healthy `:8000` + kali healthy `:2222`, Supabase activo vía `.env` de la raíz). ⬜ Manuales del usuario: decisión token gh (conservar/revocar), backup de la minisign key privada + password en gestor, prueba de instalación del MSI v3.0.1 en Windows, y **VPS** (ver § Pendiente).

> ✅ **Pack 8 — Login + activación token API** (26 Sep 2026, commit `edc7172`, CI ✅ Deploy ✅ Desktop Build ✅): el guard opt-in de Pack 4 autenticaba a cualquier visitante (el SPA hacía `set_cookie` en `/`). Ahora **la password de login ES el token configurado** (`MIRV_API_TOKEN` o `MIRV_API_TOKEN_FILE`): `POST /api/auth/login` (exempt) valida y deposita cookie httpOnly `mirv_token` (SameSite=lax, Path=/, 7 días); `POST /api/auth/logout` la borra; `GET /api/auth/status` reporta `authenticated`; `/` YA NO set-cookie; `/ws` gate con `close(code=1008)` si el guard está on y no hay cookie válida. `api_auth.py`: +`LOGIN_PATH`/`LOGOUT_PATH`/`authenticated(headers,cookies)`. Frontend: modal `#login-modal` (z-100000) + botón `#btn-auth` 🔒/🔓 (tooltip lock/logout), `runAuthGate()` al DOMContentLoaded, `window.doLogin`/`window.authToggle`, Enter en el password, i18n en/es (8 claves). VPS: activar `MIRV_API_TOKEN` en `.env` y no exponer :8000 sin Cloudflare Tunnel/acceso restrictivo. Tests `test_api_auth.py` 17→**25**; websocket gaps 20 passed; endpoints+gaps 641 passed. Suite completa: **4788 passed** ✅.

> ✅ **Pack 2 — Assessments/Scheduler pro + prod UX** (15 Sep 2026): **(1) Persistencia Supabase opt-in** — tabla `workspace_state` (key/value JSONB) + capa `backend/workspace_store.py` detrás de `MIRV_PERSIST_WORKSPACE=1` + `database.is_available()` (cache en proceso, fail-silent; registry in-memory autoritativo). `_persist()` en las 5 mutaciones de assessments (create/update/delete/add_target/remove_target) y las de scheduler (create/update/delete/advance_to_now/due_jobs) + `load_from_store()` hidratado en startup. **(2) Export/import JSON** — `GET /api/assessments/export | POST /api/assessments/import | GET /api/scheduler/export | POST /api/scheduler/import` (body `{rows, replace}` — merge o reemplazo, valida y salta inválidos, reporta imported/skipped/rejected) + botones 📥/📤 en ambas tabs. **(3) Run history en jobs** — `run_count/last_result/last_duration/last_findings/last_error` + `record_run(jid,...)`; frontend: `schedulerPollDue` marca `window._lastScheduledRun` → `finishToolOutput` llama `_reportScheduledRun` → toast `✅ Scheduled 'X' finished — N findings in Ys` + tarjeta `⚙ N runs • last: K findings in Zs`. **(4) Presets de campaña en cadena** — dropdown 🔗 Presets (Full Recon 4/Web Scan 3/SMB Audit 2/Quick Recon 2) → nuevo parámetro `start_offset_seconds` en `create_job`/`POST /api/scheduler/jobs` (None=histórico `now+interval`; offset 0 dispara en el próximo poll); jobs escalonados en cascada. **(5) Palette global Ctrl+K** — modal 12 con búsqueda en vivo sobre tabs (29), 50+ tools (⚡, categorizadas) y acciones; ↑/↓ + ↵ (⇧↵ no cierra), Esc/click-outside. **(6) Assessment activa** — `☆ Set active`/`★ Active` por card (`localStorage mirv_active_assessment`), badge ⭐ + borde ámbar; `launchTool` auto-rellena target (toast de contexto). **(7) i18n tabs nuevas** — 24 claves en/es + `applyLanguage()` (IDs, placeholders, botones, empty-states, status labels). Tests: nuevo `test_workspace_store.py` + endpoints en assessments/scheduler → **96 passed**; suite completa CI: **4672 passed** ✅. Rutas 269→**278** (workspace_state). ⬜ Manuales diferidos: `docs/CHECKLIST_DEPLOY_MANUAL.md` (MSI Desktop, Hito A VPS, Fase 7 Cloudflare).
...
2. **Desde Kali** — subir clave y preparar:
   ```bash
   ssh-copy-id -i ~/.ssh/mirv_deploy.pub root@TU_IP   # debe entrar SIN password tras esto
   # Instalar docker + clon (en el VPS, una vez):
   ...
   #   git clone https://github.com/SenorJA/dashboard-ctf.git /opt/mirv && cd /opt/mirv && git checkout main
   scp -i ~/.ssh/mirv_deploy .env root@TU_IP:/opt/mirv/.env   # el .env de la raíz local (Supabase y demás)
   ```
...
- ⬜ Backup de `desktop/.tauri/mirv-updater.key` (privada) + su password (no commitearla; está solo en GitHub Secrets `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` y en local) en el gestor de contraseñas. La `.pub` ya está embebida en `tauri.conf.json` y commiteada.
- ⬜ Prueba de instalación del `MIRV_3.0.1_x64_en-US.msi` en Windows (splash → dashboard → tray → Cerrar). Nota: si v3.0.0 estaba instalada, el cambio de identifier hace que Windows la trate como producto distinto (el update in-place real aplica para v3.0.2+).
- ⬜ Opcional: Fase 7 Cloudflare Tunnel (dominio) — andamiaje listo en `docker-compose.yml` (profile `cloudflared`).
...
### Fase D — Inspiración arquitectónica OpenExecutive (implementado ✅ 1-4, COMPLETA)
El repo `SenteLabsAI/OpenExecutive` (2.3k⭐) aporta 4 ideas para mejorar MIRV:
1. ✅ **Orchestrator multi-agente** (15 Ago 2026, commits `2cbb2b6`+`9b331df`+`0693715`) — Op Admiral convertido en orchestrator que enruta a 5 especialistas de pentest (recon, webvuln, osint, forensics, password-audit) con memoria episódica. `backend/orchestrator.py` (100% cov): `route()` por keywords (mín. hits → default recon), `execute()` auto-ruta + system prompt grounded en skill playbook (equivalente al RAG ChromaDB de OpenExecutive) + redacción de task/context antes del LLM + scope gate defensivo para red-team. 4 endpoints (`/api/orchestrator/route`, `/api/orchestrator/specialists[/{name}]`, `/api/orchestrator/hunt`). **1 Sep 2026**: hunt-* routing — `webvuln` detecta clase de vuln (`route_hunt`, mapa `_HUNT_BY_CLASS`) y groundea el LLM en el playbook `hunt-<class>`; skills ofensivas `requires_scope` degradan a base sin scope.
...
4. ✅ **Local models abstraction** (15 Ago 2026, commit `28fad54`) — panel "Model per specialist" en tab Op Admiral: provider/model/API key por especialista persistido en `localStorage['mirv_opd_models']` (override > global `_getAIConfig()`). `generatePlan` envía `opdModelFor(currentSpecialist)` al backend (precedencia args > env `MIRV_AI_*` > default local Ollama). 9 claves i18n nuevas.
